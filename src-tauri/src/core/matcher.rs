...
pub fn matches_condition(path: &Path, condition: &Condition) -> bool {
    match condition {
        ...
        Condition::SizeGreater(size) => {
            std::fs::metadata(path).map(|m| m.len() > *size).unwrap_or(false)
        }
        Condition::SizeSmaller(size) => {
            std::fs::metadata(path).map(|m| m.len() < *size).unwrap_or(false)
        }
        Condition::OlderThan(seconds) => {
            file_age_seconds(path).map(|age| age > *seconds).unwrap_or(false)
        }
        Condition::NewerThan(seconds) => {
            file_age_seconds(path).map(|age| age < *seconds).unwrap_or(false)
        }
...
mod tests {
    ...
    fn test_extension_match() {
        ...
        assert!(matches_condition(&file, &Condition::Extension("pdf".into())));
        assert!(!matches_condition(&file, &Condition::Extension("txt".into())));
    }
    ...
    fn test_glob_match() {
        ...
        std::fs::File::create(&file).unwrap();
        assert!(matches_condition(&file, &Condition::Glob("report_*.pdf".into())));
        assert!(!matches_condition(&file, &Condition::Glob("invoice_*.pdf".into())));
    }
